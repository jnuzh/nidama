...
class KoHG(KoSCCBase):
    ...
    def _do_history(self, fileuri, options, scc_handler=None):
        ...
        for revision in result['log']:
            ...
            author, encoding, bom = self._encodingSvc.getUnicodeEncodedStringUsingOSDefault(revision.get('author', ''))
            koHistory.author  = author
            koHistory.date    = revision.get('date', '')
            koHistory.action  = ''  # No action given
            message, encoding, bom = self._encodingSvc.getUnicodeEncodedStringUsingOSDefault(revision.get('message', ''))
