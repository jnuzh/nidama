...
class KoRefactoringLanguageServiceBase:
    ...
    def determineWordUsage(self, words, scimoz, koDoc, selectionStart,
        ...
        def on_get_curr_section(ciSection):
            if ciSection is None:
                start_lines["curr"] = 1
            else:
                start_lines["curr"] = ciSection.line
            ciBuf.section_from_line(self.lineStart,
                                    components.interfaces.koICodeIntelBuffer.SECTION_NEXT,
                                    False,
                                    on_get_next_section,
                                    errorCallback)

        def on_get_next_section(ciSection):
            if ciSection is None:
                next_section_start_line = scimoz.lineCount
            else:
                next_section_start_line = ciSection.line
        ...
        ciBuf.section_from_line(self.lineStart,
                                components.interfaces.koICodeIntelBuffer.SECTION_CURRENT,
                                False,
                                on_get_curr_section, errorCallback)
    ...
    def extractMethod(self, scimoz, selectionStart, selectionEnd,
        ...
        try:
            ...
            lastErrorSvc.setLastError(nsError.NS_ERROR_INVALID_ARG, ex.message)
            raise ServerException(nsError.NS_ERROR_INVALID_ARG, ex.message)
