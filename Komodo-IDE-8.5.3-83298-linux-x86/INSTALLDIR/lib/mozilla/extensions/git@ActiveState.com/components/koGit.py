...
class KoGIT(KoSCCBase):
    ...
    def _do_diff_relative(self, baseURI, relpaths, options, external, scc_handler=None):
        ...
        repodir = self.getParentDirContainingDirname(self._gitDirName, basedir)
        if not repodir:
    ...
    def _do_history(self, fileuri, options, scc_handler=None):
        ...
        result = scc_handler.log(filename, cwd=basedir, env=env)
        if result is None:
        ...
        for revision in result['log']:
            ...
            author, encoding, bom = self._encodingSvc.getUnicodeEncodedStringUsingOSDefault(revision['author'])
            koHistory.author  = author
            koHistory.date    = revision['date']
            koHistory.action  = ''  # No action given
            message, encoding, bom = self._encodingSvc.getUnicodeEncodedStringUsingOSDefault(revision['message'])
    ...
    def _do_status(self, files, recursive, options, scc_handler=None):
        ...
        if not relpaths:
            relpaths = ["."]
        repo_path = self.getParentDirContainingDirname(self._gitDirName, basedir)
        env = koprocessutils.getUserEnv()
        #print "files %r basedir %r" %(files, basedir)
            ...
            if (status_string and status_string in "CADH"):
                ...
                if status_string == 'C':
                    ...
                    if filepath not in files:
                        # Didn't ask about this file - so ignore it.
                        continue
