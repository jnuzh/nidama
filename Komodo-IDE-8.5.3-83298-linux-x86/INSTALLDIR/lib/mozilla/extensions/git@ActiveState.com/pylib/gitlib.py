...
class GIT:
    ...
    def _runCommand(self, argv, cwd=None, env=None, input=None,
                    ignore_exit_status=False, terminalHandler=None):
    ...
    def log(self, path, cwd=None, options=None, env=None):
        result = {}
        argv = [self.executable, "log"]
    ...
    def status(self, repobasedir, cwd, relpaths=None, options=None, env=None,
        ...
        if self._versionInRange(minVersion="1.7.2"):
            # --ignored was added in git 1.7.2; earlier versions means we'll not
            # be able to figure out which files are ignored (and treat them as
            # committed) :|
            argv.append("--ignored")
        ...
        for line in output.split("\0"):
            ...
            if status1 == 'A':
                ...
                files[fullpath] = { 'status': 'C',  # Changed
                                    'relpath': cwd_relpath }
            elif status1 in '?!' or status2 in '?!':
                log.debug('fullpath: %r', fullpath)
                files[fullpath] = { 'status': '?',  # Unknown or Ignored
                                    'relpath': cwd_relpath }
        ...
        if not found_results:
            ...
            if parent.startswith(repobasedir): # check that we didn't go outside
                argv = [self.executable, "ls-files", "-t", "--directory",
                        "--others", "--", leaf]
                output, error = self._runCommand(argv, cwd=parent, env=env)
                ...
                if isdir(path):
                    ...
                    if recursive:
                        for dirpath, dirnames, filenames in os.walk(cwd):
                            # Filter out directories that are not under scc control.
                            for dname in dirnames[:]: # a copy so we can modify in place
                                fullpath = join(dirpath, dname)
                                info = files.get(fullpath)
                                if info and info.get('status') == '?':
                                    # Ignored or not part of the repo - filter out.
