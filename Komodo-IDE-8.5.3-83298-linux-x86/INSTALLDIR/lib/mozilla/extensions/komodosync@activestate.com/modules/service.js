...
Cu.import("resource://services/sso.js");
...
WeaveSvc.prototype = {
  ...
  onStartup: function onStartup() {
    ...
    Auth.defaultAuthenticator = new KomodoSsoAuthenticator(id);
  ...
  _initLogs: function WeaveSvc__initLogs() {
    ...
    if (enabled) {
      ...
      let maxSize = 5485760; // 5 * 1024 * 1024 (5MB)
...
/**
 * Needs to cover three cases:
 * - login/logout, to enable/disable Sync
 * - sessionKey changed, to update credentials seemlessly
 * - account changed, to reset sync and require setup to continue service
 */
let SsoConnector = {
    onSsoLogin: function SsoConnector_onSsoLogin() {
      // The username should stay the same, as long as the email adress doesn't
      // change, in which case `onAccountChanged` would be invoked. However,
      // the username is only available while SSO is logged in, so we set it here.
      Service.username = SSOSvc.userId;
      Service.password = SSOSvc.sessionKey;
      if (Service._checkSetup() == STATUS_OK)
        Service.login();
    },
    
    onSsoLogout: function SsoConnector_onSsoLogout() {
      Service.logout();
    },
    
    onAccountChanged: function SsoConnector_onAccountChanged() {
      Service.startOver();
    },
    
    onStartup: function SsoProxy_onStartup() {
      Svc.Obs.add("services:sso:login-successful", this.onSsoLogin, this);
      Svc.Obs.add("services:sso:logout-successful", this.onSsoLogout, this);
      Svc.Obs.add("services:sso:account-changed", this.onAccountChanged, this);
