...
PasswordStore.prototype = {
  ...
  _nsLoginInfoFromRecord: function PasswordStore__nsLoginInfoRec(record) {
    if (record.formSubmitURL === "") {
      // Fix Komodo bug 90217 - shouldn't use empty string for formSubmitURL.
      this._log.debug("record had empty formSubmitURL string: " + record.hostname);
  ...
  create: function PasswordStore__create(record) {
    ...
    try {
      ...
      this._log.warn("failed to addLogin: " + record.hostname +
                     ", error: " + Utils.exceptionStr(ex));
    }
  },
  ...
  remove: function PasswordStore__remove(record) {
    ...
    try {
      ...
      this._log.warn("failed to removeLogin, hostname: " + record.hostname +
                     ", id: " + record.id +
                     ", error: " + Utils.exceptionStr(ex));
    }
  },
