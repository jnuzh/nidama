...
class KoSCCChecker(KoFileCheckerBase):
    ...
    def updateFileStatus(self, koIFile, reason):
        ...
        isUpToDate = self._lastChecked.get(diruri_cache_key, 0)
        if isUpToDate:
            dircache = self._cached_info.get(diruri_cache_key)
            if dircache is None:
            ...
            sccInfoHasChanged = (oldscc != koSccInfo)
        ...
        # The cache may contain information for multiple directories (e.g. git
        ...
        for dirpath, scc_info_from_path in dircache.items():
            ...
            if not directory in cache:
                cache[directory] = self._cached_info.get(dirpath, {})
