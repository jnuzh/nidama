...
class KoHarnessPython3Tests(sleuth.KoSleuthHarness):
    def run(self, env=None):
        if env is None:
            env = self._get_non_komodo_env()
        sleuth.fixLanguageIncludePath(env, "python3ExtraPaths", "PYTHONPATH")
        return sleuth.KoSleuthHarness.run(self, env)

class Python3_Unittest_SleuthHarness(sleuth.KoSleuthHarnessRunner):
    def initialize(self, cmd, launch_dir=None, executable=None):
        test_dir = os.path.abspath(os.path.dirname(__file__))
        launch_dir = launch_dir or test_dir
        python_exe = executable or "python3" # Not which.which!
        ...
        if cmd:
            ...
            self._harness = KoHarnessPython3Tests(cmd, launch_dir)
        else:
            self._initial_msgs = [
...
harness_name = "Python3 - unittest"
compatible_with_v_4_3 = True

def register(sleuthManager):
    sleuthManager.register('Python3', harness_name, Python3_Unittest_SleuthHarness,
