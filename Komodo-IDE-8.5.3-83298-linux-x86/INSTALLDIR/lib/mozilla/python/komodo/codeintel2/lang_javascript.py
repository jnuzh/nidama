...
class JavaScriptLangIntel(CitadelLangIntel,
    ...
    def sections_from_blob(self, blob):
        def _sections_from_scope(scope):
            for child in scope:
                if (child.tag == "scope"
                    # JavaScript CIX uses <variable> tags with children
                    # for namespaces via anonymous classes/objects.
                    or child.tag == "variable" and len(child)):
                    pass
                else:
                    continue
                child.set("level", scope.get("level") + 1)
                section = Section(
                    child.get("name"),
                    int(child.get("line")),
                    lang,
                    level=child.get("level"),
                    type=child.get("ilk") or "namespace",
                    id=child.get("name"))
                yield section
                for s in _sections_from_scope(child):
                    yield s

        lang = blob.get("lang")
        blob.set("level", -1)
        for s in _sections_from_scope(blob):
            yield s
