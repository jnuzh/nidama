...
class CoreHandler(CommandHandler):
    ...
    def do_get_sections(self, request, driver):
        buf = driver.get_buffer(request)
        ...
        for section in sections:
            data = {"title": section.title,
                    "line": section.line,
                    "lang": section.lang}
            for attr in "type", "level", "id":
                if getattr(section, attr, None) is not None:
                    data[attr] = getattr(section, attr)
            results.append(data)
        driver.send(sections=results)
    ...
    def do_get_scopes(self, request, driver):
        buf = driver.get_buffer(request)
        pos = request['pos']
        scopes = self._do_get_scopes_aux(buf, pos)
        driver.send(scopes=scopes)

    def _do_get_scopes_aux(self, buf, pos):
        try:
            scoperef = buf.scoperef_from_pos(pos)
        except AttributeError:
            # Not all languages lead to scoperefs
            return []
        if scoperef is None:
            #log.debug("scoperef is None at pos %d", pos)
            return []
        blob, lpath = scoperef
        if not lpath:
            #log.debug("lpath is None at pos %d", pos)
            return []
        scopes = []  # Outer first, innermost last
        scope = blob
        while scope and lpath:
            target = lpath[0]
            del lpath[0]
            for child in scope.getchildren():
                if child.get("name") == target:
                    #log.debug("Raw ilk: %r", child.get("ilk"))
                    lineStart = int(child.get("line", 0))
                    lineEnd = child.get("lineend")
                    if lineEnd is None:
                        #log.debug("%s: lineEnd is None", target)
                        lineEnd = lineStart
                    else:
                        lineEnd = int(lineEnd)
                    data = {"name": child.get("name"),
                            "ilk":  child.get("ilk"),
                            "lineStart": lineStart,
                            "lineEnd": lineEnd,
                            }
