...
__all__ = [
    "Trigger", "Definition", "Section", "CILEDriver", "Evaluator",
    "EvalController", "LogEvalController", "Section",
...
class Section(object):
    if _xpcom_:
        _com_interfaces_ = [components.interfaces.koICodeIntelSection]

    # [Required] This is the text that shows in the section menulist.
    title = None

    # [Required] The (1-based) line number of the start of the section.
    line = 0

    # [Required] The language this section belongs to. This will differ in
    # multi-lang documents. This might be used for filtering of the
    # section list.
    lang = None

    # [Optional] An string for the "type" of section. This is used to
    # determine an image to show for this section in the UI. This is the
    # same set of type/image names used for autocomplete results.
    type = None

    # [Optional] A numeric tree level. E.g. a top-level class would be
    # level 0, a function in that class would be level 1. This is possibly
    # used to show structure in the section menulist. Defaults to 0.
    # Individual languages need not define levels.
    level = 0

    # [Optional] A (possibly code-friendly) identifier. The intended usage
    # would be for opening an jumping to that section:
    #       $ komodo foo.py#myfunc
    #       $ komodo foo.xml#this-id
    # Can be None if there isn't a reasonable way to have an id. Ideally
    # these are unique (to be more useful they should be), but allowed to not
    # be.
    id = None
    
    def __init__(self, title, line, lang, type=None, level=0, id=None):
        self.title = title
        self.line = line
        self.lang = lang
        self.type = type
    ...
    def __repr__(self):
        if self.id:
            return "<Section %s@%d: %s %s %r>" % (
                self.id, self.line, self.lang, self.type, self.title)
        else:
            return "<Section @%d: %s %s %r>" % (
                self.line, self.lang, self.type, self.title)

    def pprint(self, stream=sys.stdout):
        """Print a convenient representation for a printing a list
        of sections. I.e. use `self.level` to show structure, etc.
        """
        WIDTH = 79
        data = {
            "line": self.line,
        ...
        template = "%(line)5d: %(indent)s%(type)s %(title)s"
        prefix = template % data
