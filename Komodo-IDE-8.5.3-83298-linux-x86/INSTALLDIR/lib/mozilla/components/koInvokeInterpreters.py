...
class koInterpreterInvocation(object):
    ...
    def __init__(self):
        ...
        #XXX Refactor with koRunService.unprocessed.py
    ...
    def getDefaultPreferences( self ):
        cat = Cc["@activestate.com/koPreferenceSet;1"].createInstance()
        ...
        inst = Cc["@activestate.com/koPreferenceSet;1"].createInstance()
    ...
    def _getenv(self):
        ...
        """
        # 1. the system environment
        mergedEnvStrs = self.currentInstancePrefs.getStringPref('systemEnvironment')
        mergedEnvStrs = mergedEnvStrs.strip().split("\n")

        # 2. the default CGI environment
        if self._usecgi:
            cgiEnvStrs = self.currentInstancePrefs.getStringPref('CGIEnvironment')
            cgiEnvStrs = cgiEnvStrs.strip().split("\n")
            mergedEnvStrs = self._environUtils.MergeEnvironmentStrings(
                mergedEnvStrs, cgiEnvStrs)
            
        # 3. the user environment settings
        userEnvStrs = self.currentInstancePrefs.getStringPref('userEnvironment')
        if userEnvStrs:
        ...
        # 4. the user CGI environment settings
        if self._usecgi:
            userCgiEnvStrs = self.currentInstancePrefs.getStringPref('userCGIEnvironment')
            if userCgiEnvStrs:
...
class koPythonCommonInvocation(koInterpreterInvocation):
    ...
    def _getExecutableArgs(self, executable):
        cmddata = self.currentInstancePrefs.getStringPref('executable-params')
        executableArgs = _line2argv(cmddata)
        if '-u' not in executableArgs and \
           os.path.basename(executable).find('jython') == -1 and \
           self.currentInstancePrefs.getBooleanPref('unbuffered-handles'):
    ...
    def _getDBGPClientDriverArgs(self):
        ...
        if self._pythonVersion >= 3.0:
            ...
            pyPart = "python"
        logLevel = self.effectivePrefs.getStringPref(pyPart + "_debuggerLogLevel")
        driverArgs.append("-l")
        driverArgs.append(logLevel) # one of 'DEBUG', 'WARN', etc.
        if logLevel != "NOTSET":
            logFile = self.effectivePrefs.getStringPref(pyPart + "_debuggerLogPath")
            if logFile:
                driverArgs.append("-f")
    ...
    def _getScriptArgs(self):
        ...
        if len(scriptArgs) < 1:
            cmddata = self.currentInstancePrefs.getStringPref('params')
...
class koPerlInvocation(koInterpreterInvocation):
    ...
    def _getExtraPerlOptions(self):
        ...
        if '-w' not in ret and '-W' not in ret and '-X' not in ret:
            warnings = self.currentInstancePrefs.getStringPref('warnings')
            if warnings == "enabled":
                ret.append("-w")
            elif warnings == "all":
                ret.append("-W")
    ...
    def _getPerlIncArgv(self):
        ...
        if self.effectivePrefs.hasPref("perlExtraPaths"):
            perlExtraPaths = self.effectivePrefs.getStringPref("perlExtraPaths")
        if perlExtraPaths:
            if sys.platform.startswith("win"):
...
class koPHPInvocation(koInterpreterInvocation):
    ...
    def _determineExecutable(self):
        # use pref'd default if one has been chosen
        phpInterpreterType = self.currentInstancePrefs.getStringPref('phpInterpreterType')
        phpInfo = self.languageService.\
           getLanguageService(Ci.koIAppInfoEx)
...
class koTclInvocation(koInterpreterInvocation):
    ...
    def _launchForRun(self):
        ...
        script = self.currentInstancePrefs.getStringPref('filename')
        scriptArgs = self._getScriptArgs()
        argv = [executable] + execArgs + [script] + scriptArgs
...
# This class is currently unreachable, as all the UI code
# has been commented out.
class koJavaScriptInvocation(koInterpreterInvocation):
    ...
    #XXX There are no JavaScript prefs, although we might want to give ability
    # to select a Firefox executable

    def get_supportedInvokeTypes(self):
        return INVOKE_INTERACTIVE

    def __init__(self):
        koInterpreterInvocation.__init__(self)
        # No need to create a prefs-observer as there aren't any JS prefs
        
    def _determineExecutable(self):
        koWebbrowser = Cc['@activestate.com/koWebbrowser;1'] \
            .getService(Ci.koIWebbrowser)
        firefoxes = koWebbrowser.get_firefox_paths()
        if len(firefoxes) == 0:
            log.error("No Firefox browser could be found.")
            return ""
        # Same as pref-javascript.js, return first if more than one
        #XXX Add prefered firefox to prefs
        return firefoxes[0]
    
    def _launchForInteractive(self):
        executable = self._determineExecutable()
        if not executable:
            log.debug("Can't find a JavaScript executable")
            return
        cmdPort = self.currentInstancePrefs.getLongPref("debugger.listener-port")
        argv = ([str(executable),
                 '-jsdbgp',
        ...
        cwd = os.getcwd()
        env = self._getenv()
        log.debug("About to launch the interactive shell: (%s, %s, %s)", argv, cwd, env)
        self._doLaunch(argv, cwd, env)
        
    # Things expected by the environment:
    def _do_validate(self, invokeType):
        if invokeType == INVOKE_INTERACTIVE:
            return 1, None, None
        return 0, 42, "Can't debug JavaScript within Komodo; use Firefox"
