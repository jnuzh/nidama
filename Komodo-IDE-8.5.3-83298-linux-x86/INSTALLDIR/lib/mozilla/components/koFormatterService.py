    ...
    def getFormatterForContext(self, context):
        ...
        if (context.prefset.hasPref('configuredFormatters')):
            ...
            for i in range(prefs.length):
                pref = prefs.getPref(i)
                if pref.getStringPref("lang") == lang:
                    formatter = self.getFormatterWithName(pref.getStringPref("formatter_name"))
                    if formatter:
                        # Update the context prefset for the specific formatter
                        # configuration.
                        context.formatter_prefset = pref
