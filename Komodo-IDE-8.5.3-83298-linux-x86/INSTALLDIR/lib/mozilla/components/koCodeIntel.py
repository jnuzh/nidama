...
class KoCodeIntelScope:
    _com_interfaces_ = [Ci.koICodeIntelScope]
    _reg_clsid_ = "{4a0434e0-a6c2-4df5-a41b-56caaa8f6570}"
    _reg_contractid_ = "@activestate.com/koCodeIntelScope;1"
    _reg_desc_ = "Komodo Code Intelligence Scope"

    def init(self, name, ilk, lineStart, lineEnd):
        self.name = name
        self.ilk = ilk
        self.lineStart = lineStart
        self.lineEnd = lineEnd

def makeScope(name, ilk, lineStart, lineEnd):
    scope = Cc["@activestate.com/koCodeIntelScope;1"].\
            createInstance(Ci.koICodeIntelScope)
    UnwrapObject(scope).init(name, ilk, lineStart, lineEnd)
    return scope
...
class KoCodeIntelBuffer(object):
    ...
    SECTION_PREVIOUS = Ci.koICodeIntelBuffer.SECTION_PREVIOUS
    SECTION_CURRENT = Ci.koICodeIntelBuffer.SECTION_CURRENT
    SECTION_NEXT = Ci.koICodeIntelBuffer.SECTION_NEXT

    def section_from_line(self, line, whence, wrap, callback, errorCallback=None):
        @ProxyToMainThreadAsync
        def invoke_callback(section):
            try:
                if hasattr(callback, 'onGetSection'):
                    callback.onGetSection(section)
                else:
                    callback(section)
            except:
                self.log.exception("Error calling section_from_line callback")
        def got_sections(request, response):
            if not response.get("success"):
                if errorCallback:
                    msg = (response.get("message")
                           or ("section_from_line failed at line %d" % (line,)))
                    self._do_error_callback(errorCallback, msg)
                else:
            ...
            idx = bisect.bisect_right(lines, line) - 1 + whence
            if idx < 0:
                if wrap:
                    invoke_callback(KoCodeIntelSection(sections[-1]))
                else:
                    invoke_callback(None)
            elif idx >= len(sections):
                if wrap:
                    invoke_callback(KoCodeIntelSection(sections[0]))
                else:
                    invoke_callback(None)
            else:
                invoke_callback(KoCodeIntelSection(sections[idx]))

        self.send(command="get-sections",
                  path=self.path,
                  language=self.lang,
                  text=self.doc.buffer if self.doc else None,
                  env=self.env,
                  callback=got_sections)

    def section_idx_from_line(self, line, callback, errorCallback=None):
        @ProxyToMainThreadAsync
        def invoke_callback(idx):
            try:
                callback.onGetIndex(idx)
            except:
                self.log.exception("Error calling section_idx_from_line callback")
        def got_sections(request, response):
            if not response.get("success"):
                if errorCallback:
                    msg = (response.get("message")
                           or ("section_idx_from_line failed at line %d" %
                               (line,)))
                    self._do_error_callback(errorCallback, msg)
                else:
                    invoke_callback(-1)
            ...
            sections = response["sections"]
            lines = [s["line"] for s in sections]
            invoke_callback(bisect.bisect_right(lines, line) - 1)
    ...
    def get_sections(self, callback, errorCallback=None):
        try:
            callback = ProxyToMainThreadAsync(callback
                            .QueryInterface(Ci.koICodeIntelGetSectionsCallback)
                            .onGetSections)
        except AttributeError:
            pass # not XPCOM
        ...
        def got_sections(request, response):
            if not response.get("success") and errorCallback:
                msg = response.get("message") or "get_sections failed"
                self._do_error_callback(errorCallback, msg)
                return
            try:
                callback(list(KoCodeIntelSection(s)
                              for s in response.get("sections", [])))
            except:
                self.log.exception("Error calling get_sections callback")
    ...
    def get_scope_list(self, koDoc, pos, callback, errorCallback=None):
        @ProxyToMainThreadAsync
        def invoke_callback(scopes):
            try:
                callback.onGetScopes(scopes)
            except:
                self.log.exception("Error calling get_scope_list callback")
        def got_scopes(request, response):
            if not response.get("success"):
                if errorCallback:
                    msg = (response.get("message")
                           or ("section_idx_from_line failed at line %d" %
                    ...
                    invoke_callback(None)
                return
            raw_scopes = response["scopes"]
            scopes = [makeScope(scope['name'], scope['ilk'],
                                scope['lineStart'], scope['lineEnd'])
                      for scope in raw_scopes]
            invoke_callback(scopes)

        self.send(command="get-scopes",
                  path=self.path,
                  language=self.lang,
                  text=self.doc.buffer if self.doc else None,
                  pos=pos,
...
class KoCodeIntelSection(object):
    _com_interfaces_ = [Ci.koICodeIntelSection]

    def __init__(self, data):
        """Construct the section from a dictionary"""
        self.__dict__.update(data)

class KoTACSectionsSearch(KoTACSearch):
    """Textbox autocomplete for codeintel "sections" widget in statusbar."""
    _com_interfaces_ = [Ci.nsIAutoCompleteSearch]
    _reg_clsid_ = "{ca152305-bd11-4df0-8edc-0215cfae7565}"
    _reg_contractid_ = "@mozilla.org/autocomplete/search;1?name=sections"
    _reg_desc_ = "Komodo document sections textbox autocomplete search"

    def __init__(self):
        KoTACSearch.__init__(self)
        self.log = log.getChild("autocomplete")
        self.debug = self.log.debug
        self.viewSvc = Cc["@activestate.com/koViewService;1"]\
                .getService(Ci.koIViewService)

    @LazyClassAttribute
    def prefs(self):
        return Cc["@activestate.com/koPrefService;1"]\
                    .getService(Ci.koIPrefService).prefs

    def startSearch(self, searchString, searchParam, previousResult, listener):
        self.debug("startSearch: %r", searchString)

        result = Cc["@activestate.com/autocomplete/result;1"] \
            .createInstance(Ci.koIAutoCompleteResult)
        result.init(searchString)

        try:
            currView = self.viewSvc.currentView.QueryInterface(Ci.koIScintillaView)
            ciBuf = UnwrapObject(currView.koDoc.ciBuf)
        except Exception, ex:
            result.setFailed("unexpected error: %s" % ex)
            listener.onSearchResult(self, result)
            return

        scimoz = currView.scimoz
        currLine = scimoz.lineFromPosition(scimoz.currentPos) + 1 # 0-based -> 1-based
        ciBuf.get_sections(functools.partial(self.on_have_sections, searchString,
                                             currLine, result, listener))

    def on_have_sections(self, searchString, line, result, listener, sections):
        # Second value is whether to be case-sensitive in filtering.
        filterWords = [(w, w.lower() != w) for w in searchString.strip().split()]
        lines = [s.line for s in sections]
        currIdx = bisect.bisect_right(lines, line) - 1
        currSection = (currIdx != -1 and sections[currIdx] or None)
        showContext = False

        if filterWords:
            # Filter on the section title.
            filteredSections = []
            for s in sections:
                titleLower = s.title.lower()
                for w, caseSensitive in filterWords:
                    if (caseSensitive and w not in s.title
                        or not caseSensitive and w not in titleLower):
                        break
                else:
                    filteredSections.append(s)

            levelSum = sum(s.level for s in sections)
            if levelSum:
                # Not all level=0, hence we might want to provide
                # context for deeper sections. Build
                # `parentFromSection` to assist with determining
                # context.
                showContext = True
                parentFromIdSection = {}
                parentStack = []
                for s in sections:
                    while parentStack and parentStack[-1].level >= s.level:
                        parentStack.pop()
                    parentStack.append(s)
                    if len(parentStack) > 1:
                        parentFromIdSection[id(s)] = parentStack[-2]
        else:
            filteredSections = sections

        if self.prefs.getLongPref("codeintel_sectionlist_sort_order") == 1: # Sort alphabetically.

            # Sort the sections list by the name (case insensitive).
            def recursively_sort_sections(sections, level=0):
                """Sort the provided (sub) sections at this provided level."""
                # Note: sections_for_level is a list of 4 items:
                #    0: section title - lowercased
                #    1: section
                #    2: children start position
                #    3: children end position
                sections_for_level = []
                for i in range(len(sections)):
                    section = sections[i]
                    if section.level == level:
                        if sections_for_level:
                            sections_for_level[-1][-1] = i
                        sections_for_level.append([section.title.lower(), section, i+1, i+1])
                sorted_sections = []
                if sections_for_level:
                    sections_for_level[-1][-1] = i
                    sections_for_level.sort(key=operator.itemgetter(0))
                    sorted_sections = []
                    for section_title, section, i_start, i_end in sections_for_level:
                        sorted_sections.append(section)
                        sorted_sections += recursively_sort_sections(sections[i_start:i_end+1], level+1)
                return sorted_sections

            filteredSections = recursively_sort_sections(filteredSections)

        for s in filteredSections:
            # Mozilla limitation: Only one tree cell property allowed.
            # This is a basic limitation of the textbox-autocomplete
            # API and system. This means we need a huge number of CSS
            # declaractions, and if the section type is unknown then
            # all styling is lost (See "GetCellProperties()" in
            # toolkit/components/autocomplete/src/nsAutoCompleteController.cpp.)
            if s is currSection:
                style = "ci-sections-curr-%s-%s" % (s.type, s.level)
            else:
                style = "ci-sections-%s-%s" % (s.type, s.level)

            if showContext and id(s) in parentFromIdSection:
                title = "%s   [%s]" % (
                    s.title, parentFromIdSection[id(s)].title)
            else:
                title = s.title
            result.addMatch(
                title,              # value
                # The sections autocomplete widget relies on getting
                # the line number for the jump from the comment field.
                str(s.line),        # comment
                style,              # style
                False,              # isDefault
                #TODO: Can 'image' here be used to replace all the CSS
                #      list-image-style hacks that we have for this?
                None)               # image
